...
Sunshine supports connecting a device in controller-only mode. This allows you to use a second device as a dedicated 
controller while another device handles the video and audio streaming. 

There are two ways to enable controller-only mode:

1. **Through the Web UI:**
   - Go to the Applications section in Sunshine's web UI
   - Edit an existing application or create a new one
   - Enable the "Controller-only Mode" checkbox
   - Save the application

2. **By adding a parameter when connecting:**
   - Add the `controllerOnly=1` parameter when connecting from a Moonlight client

In Moonlight clients that don't support custom parameters yet, you can use a modified version of Moonlight that includes 
this parameter in the connection request.

Example usage scenarios:
- Using a phone as a controller while streaming video to a TV or monitor
- Allowing a friend to join your game with controller input only
- Adding accessibility options by having separate input and output devices
